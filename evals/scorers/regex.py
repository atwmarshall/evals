...
class RegexScorer:
    ...
    `expected` and `ctx` are not used — this scorer checks format/content presence,
    not equality against a known answer.
...
class MultiRegexScorer:
    ...
    `expected` and `ctx` are not used — this scorer checks content presence across
    multiple required patterns, e.g. verifying a response covers several required points.
